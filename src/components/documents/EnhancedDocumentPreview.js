//src/components/documents/EnhancedDocumentPreview.js
...
const PreviewModal = ({ document, onClose }) => {
  ...
  useEffect(() => {
    if (document.type.startsWith('image/')) {
      ...
      setLoading(false); // PDF preview handled by browser
    } else {
      setLoading(false);
    }
  }, [document]);
        ...
        <div className="p-4 border-t">
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-sm text-gray-500">
                Last modified: {new Date(document.file.lastModified).toLocaleDateString()}
              </p>
            </div>
            <button
              onClick={() => {
